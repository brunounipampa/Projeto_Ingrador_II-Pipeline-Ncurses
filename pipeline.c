...
void executarCicloInstrucao(PC *pc, BancoRegistradores *banco_registradores, MemoriaUnica *memoria_unica) {
    ...
    char instrucao_string[TAM_INSTRUCAO + 1];

    if (memoria_unica->instrucoes[pc->endereco_atual] == 0) {
        // ...
    } else {
        char *instrucao_string = (char *)malloc(64 * sizeof(char));
	snprintf(instrucao_string, 64, "%63s", (char *)memoria_unica->instrucoes[pc->endereco_atual]);
    ...
    WINDOW *pipeline_win = newwin(10, 30, 1, 50);
    box(pipeline_win, 0, 0);
    mvwprintw(pipeline_win, 1, 1, "Pipeline MIPS");
    mvwprintw(pipeline_win, 2, 1, "Fetch: %s", instrucao_string);
    ...
    sleep(1); // Pausa por 1 segundo
            ...
            switch (estado) {
                case 0: // ETAPA DE BUSCA DA INSTRUÇÃO
                    ...
                    estado = 1;
                    printf("\n");
                    mvwprintw(pipeline_win, 3, 1, "Decode: %s", instrucao_string);
            	    wrefresh(pipeline_win);
                    sleep(1); // Pausa por 1 segundo
                    estado = 1;
                    break;
                ...
                case 1: // ETAPA DE DECODIFICAÇÃO DA INSTRUÇÃO E BUSCA DOS REGISTRADORES
                    ...
                    mvwprintw(pipeline_win, 4, 1, "Execute: %s", instrucao_string);
		    wrefresh(pipeline_win);
		    sleep(1); // Pausa por 1 segundo
		    estado = 2;
		    break;
        ...
        case 2: // EXECUTE (TIPO I)
            ...
            mvwprintw(pipeline_win, 5, 1, "Memory: %s", instrucao_string);
            wrefresh(pipeline_win);
            sleep(1); // Pausa por 1 segundo
            estado = 3;
            break;
        ...
        case 4: // WRITEBACK (LW)
            ...
            printf("\n");
            mvwprintw(pipeline_win, 6, 1, "Write Back: %s", instrucao_string);
            wrefresh(pipeline_win);
            sleep(1); // Pausa por 1 segundo
            estado = 0;
            break;
...
void imprimirMemoriaUnica(MemoriaUnica *memoria_unica) {
    WINDOW *mem_instrucao_win = newwin(10, 20, 1, 1);
    WINDOW *mem_dados_win = newwin(10, 20, 1, 25);
    mvwprintw(mem_instrucao_win, 1, 1, "Memória de Instruções:");
    for (int i = 0; i < TAM_MEMORIA_INSTRUCAO; i++) {
        mvwprintw(mem_instrucao_win, i + 2, 1, "Instrucao[%d]: %s", i, memoria_instrucao[i]);
    }
    wrefresh(mem_instrucao_win);

    mvwprintw(mem_dados_win, 1, 1, "Memória de Dados:");
    for (int i = 0; i < TAM_MEMORIA_DADOS; i++) {
        mvwprintw(mem_dados_win, i + 2, 1, "Dados[%d]: %d", i, memoria_dados[i]);
    }
    wrefresh(mem_dados_win);
}

void imprimirRegistradores(BancoRegistradores *banco_registradores) {
    WINDOW *registradores_win = newwin(10, 20, 1, 50);
    mvwprintw(registradores_win, 1, 1, "Conteudo do banco de registradores:");
    for (int i = 0; i < TAM_REGISTRADORES; i++) {
        mvwprintw(registradores_win, i + 2, 1, "R%d: %d", i, banco_registradores->registradores[i]);
    }
    wrefresh(registradores_win);
}
...
void inicializePilha(descritor *desc) {
    if (desc != NULL) {
        ...
        // Tratar o erro, por exemplo, imprimir uma mensagem de erro
        printf("Erro: Ponteiro desc é nulo.\n");
    }
}
